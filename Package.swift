...
let package = Package(
  ...
  targets: [
    ...
    .executableTarget(
      ...
      linkerSettings: [
        // macOS 27 beta (26A5353q) runtime MSL compiler fails to resolve MPP / metal_tensor
        // symbols in compiled binaries (works in `swift script.swift` JIT mode).
        // Root cause: the runtime MSL compiler only checks MetalPerformancePrimitives.framework
        // headers when the binary has been linked against that framework at load time, even
        // though the framework ships no host dylib (only MSL headers + libTensorOps.rtlib).
        // The private GPUCompiler.framework must also be loaded for the lowering pipeline.
        // Workaround: force both onto the link line. Tracking: macOS 27 Golden Gate Beta 2
        // (26A5368g, pending update) updates libTensorOps.rtlib and metal_tensor headers.
        .unsafeFlags([
          "-F", "/System/Library/PrivateFrameworks",
          "-framework", "GPUCompiler",
